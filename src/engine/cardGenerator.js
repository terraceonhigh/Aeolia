...
export function generateSituationCards(snapshot, playerCore, names, frontier, opts) {
  ...
  // ════════════════════════════════════════════════════════
  // Three-slot category system: each category independently
  // selects at most one card, guaranteeing representation
  // from strategic, governance, and economic/environmental
  // concerns. Fixes priority starvation of late-registered
  // card types (minerals, epidemics, institutional reform).
  // ════════════════════════════════════════════════════════

  // ── SLOT 1: STRATEGIC (military, expansion, diplomacy, minerals) ──

  function generateStrategicCard() {
    // Tech lag / tech edge
    ...
    if (frontier.length > 0 && activeFocus !== 'expand') {
    ...
    // Homeland only (very early game)
    if (ps.territory <= 1 && frontier.length > 0) {
    ...
    // Naphtha / carbon resource alert
    if (ps.tech >= 4.5 && ps.tech < 7.5 && frontier.length > 0) {
    ...
    // Pyra alert (pre-scramble)
    if (ps.tech >= 7.0 && ps.tech < 9.5 && frontier.length > 0) {
    ...
    // Rogue Aircraft Alert (nuclear era)
    if (ps.tech >= 9.0 && contactedCores.length > 3 && tick % 11 === 4) {
  ...
  // ── SLOT 2: GOVERNANCE (sovereignty, religion, institutions) ──

  function generateGovernanceCard() {
    // Sovereignty strain
    if (ps.territory > 1) {
      const unstable = [];
    ...
    // Colonial resistance (Scott 1985)
    if (ps.territory > 1) {
    ...
    // Institutional Reform (Acemoglu-Robinson)
    {
      ...
      if (playerExtractiveness > 0.30) {
        const reformSeed = hashStr('institutional_reform' + Math.floor(tick / 5));
  ...
  // ── SLOT 3: ECONOMIC / ENVIRONMENTAL (trade, culture, disease, resources) ──

  function generateEconomicCard() {
    // Tech decay alert (most urgent economic signal)
    if (prevTech !== null && ps.tech < prevTech - 0.05) {
    ...
    // Piracy warning
    if (contactedCores.length > 5 && ps.tech >= 3.0 && ps.tech < 8.0 && tick % 9 === 3) {
    ...
    // Saak oil displacement
    if (ps.tech >= 7.0 && contactedCores.length > 0 && tick % 11 === 4) {
    ...
    // Navigator guild tension
    if (ps.tech >= 7.0 && ps.tech < 8.5 && tick % 13 === 7) {
    ...
    // Malaria medical breakthrough
    if (!malariaUnlocked && ps.tech >= 6.0 && ps.territory > 1) {
    ...
    // Cultural Freeze Warning (Axelrod)
    {
      ...
      if (cposArr.length > playerCore && cposArr[playerCore]) {
        ...
        if (maxFreezeRisk > 0.70 && freezeRiskPartner !== null) {
          ...
          const freezeSeed = hashStr('cultural_freeze' + Math.floor(tick / 6) + freezeRiskPartner);
          return {
            id: `cultural_freeze_${Math.floor(tick / 6)}_${freezeRiskPartner}`,
            icon: '❄',
  ...
  // ── Assemble cards from all three slots ──
  return [
    generateStrategicCard(),
    generateGovernanceCard(),
    generateEconomicCard(),
  ].filter(Boolean);
}
