// ═══════════════════════════════════════════════════════════
// settlements.js — Settlement detection from geography + politics
// ═══════════════════════════════════════════════════════════

import { ISLAND_MAX_HEIGHT } from './constants.js';

export function detectSettlements(archs, history) {
  const settlements = [];
  for (let a = 0; a < archs.length; a++) {
    ...
    // Secondary port towns
    for (let i = 0; i < ar.peaks.length; i++) {
      if (usedPeaks.has(i)) continue;
      const pk = ar.peaks[i];
      if (pk.h < ISLAND_MAX_HEIGHT * 0.18) continue;
