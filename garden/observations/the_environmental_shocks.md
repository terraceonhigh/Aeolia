...
The simulation does not model the Davis finding specifically — it does not differentiate between crop failures in well-administered territories (which have distribution infrastructure) and crop failures in extractively administered territories (which may not). All polities face the same crop failure probability and recovery rate, regardless of their administrative structure. This is a known gap: the historical record strongly suggests that administrative quality and commercial access affect how severely crop failures translate into mortality and economic disruption. A more detailed model would allow extractive institutions to amplify crop failure impacts, which is one of the mechanisms through which the AR extractiveness penalty manifests in historical data.
